...
# creating the frames in the window
    ...
    def startpage(self):
        ...
        self.heading = Label(self.main, text="Hospital Management System", font=(
            'Centaur 20 bold'), fg='black', bg="grey", relief=SUNKEN)
        self.heading.place(x=60, y=20)

        # name

        self.name = Label(self.main, text="Patients Name",
                          font=('arial 12 bold'), bg="lightblue")
        self.name.place(x=0, y=110)

        # age

        self.age = Label(self.main, text="Age", font=(
            'arial 12 bold'), bg="lightblue")
        self.age.place(x=0, y=155)

        # gender
        Label(self.main, text="Gender", font=('arial 12 bold'),
              bg="lightblue").place(x=0, y=210)
        a = Radiobutton(self.main, text="Male", padx=20, font="ariel 10 bold",
                        variable=self.v, value=1, bg="lightblue").place(x=130, y=210)
        b = Radiobutton(self.main, text="Female", padx=20, font="ariel 10 bold",
                        variable=self.v, value=2, bg="lightblue").place(x=220, y=210)

        # location

        self.time = Label(self.main, text="Location",
                          font=('arial 12 bold'), bg="lightblue")
        self.time.place(x=0, y=255)

        # phone

        self.phone = Label(self.main, text="Contact Number",
                           font=('arial 12 bold'), bg="lightblue")
...
# show log
       ...
       #  display the logs in our frame
        ...
        self.logs = Label(self.main, text="Total\n Appointments", font=(
            'arial 10 bold'), fg='black', bg="lightblue")
        self.logs.place(x=340, y=320)
        self.logs = Label(self.main, text=" " + str(self.final_id),
...
# funtion to call submit button
    ...
    def showdetails(self):
        ...
        count1 = 0
        count2 = 0
        clmnname = ['App no', 'name', 'age',
                    'gender', 'location', 'contact no']
        for i in range(len(clmnname)):
            Label(self.showdetailsframe, text=clmnname[i], font="ariel 12 bold").grid(
                row=0, column=i*2)
    ...
    def updatee(self):
        ...
        self.id = Label(self.updateframe, text="Search Appoinment Number To Update", font=(
            'arial 12 bold'), fg="red")
    ...
    def update1(self):
        ...
        self.uname = Label(
            self.updateframe, text="Patient's Name", font=('arial 14 bold'))
        ...
        self.ugender = Label(
            self.updateframe, text="Gender", font=('arial 14 bold'))
        self.ugender.place(x=0, y=220)

        self.ulocation = Label(
            self.updateframe, text="Location", font=('arial 14 bold'))
        self.ulocation.place(x=0, y=260)

        self.uphone = Label(
            self.updateframe, text="Phone Number", font=('arial 14 bold'))
...
# buttons for update and delete
        self.update = Button(self.updateframe, text="Update", font="aried 12 bold",
                             width=10, height=1, bg='lightgreen', command=self.update2)
    ...
    def update2(self):
        ...
        c.execute(query, (self.var1, self.var2, self.var3,
                  self.var4, self.var5, self.idnet.get(),))
        conn.commit()
    ...
    def deletee(self):
        ...
        self.id = Label(self.deleteframe, text="Search Appoinment Number To Delete", font=(
            'arial 12 bold'), fg="red")
        self.id.place(x=0, y=12)
    ...
    def delete1(self):
        ...
        # creating the update form
        self.uname = Label(
            self.deleteframe, text="Patient's Name", font=('arial 14 bold'))
        self.uname.place(x=0, y=140)
        ...
        self.ugender = Label(
            self.deleteframe, text="Gender", font=('arial 14 bold'))
        self.ugender.place(x=0, y=220)

        self.ulocation = Label(
            self.deleteframe, text="Location", font=('arial 14 bold'))
        self.ulocation.place(x=0, y=260)

        self.uphone = Label(
            self.deleteframe, text="Phone Number", font=('arial 14 bold'))
        self.uphone.place(x=0, y=300)
        # entrys
